...
# Set global float formatting for pandas to prevent scientific notation
pd.options.display.float_format = '{:,.2f}'.format

# Suppressing FutureWarnings regarding pandas deprecations and UserWarnings
...
def format_number(num):
    """Format number to have two decimal places or '.00' if an integer."""
...
class RegressionApp:
    def __init__(self):
        self.df = None
        self.variables = []
        self.scenarios = predefined_years.copy()
    ...
    def show_variable_selection(self):
        ...
        self.variables = self.df.columns[2:].tolist()  # Assuming variables start from column C onwards
        num_combinations = sum([len(list(itertools.combinations(self.variables, i))) for i in range(1, len(self.variables) + 1)])
        self.total_regressions = 5 * num_combinations

        st.subheader(f"{len(self.variables)} variables found.")
        st.subheader(f"Regression will create {num_combinations} variable combinations (Total Regressions: {self.total_regressions}).")
    ...
    def run_regression_scenarios(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = [executor.submit(self.process_scenario, scenario_name, self.scenarios[scenario_name]) for scenario_name in self.scenarios]
    ...
    def process_scenario(self, scenario_name, years):
        ...
        for tab, (scenario_name, scenario_results) in zip(tabs, all_results[:num_tabs]):
            with tab:
                ...
                for result in scenario_results:
                    ...
                    summary_data.append(['', 'Multiple R', format_number(model.rsquared ** 0.5)])
                    summary_data.append([f"S{idx}R^2", 'R Square', format_number(model.rsquared)])
                    summary_data.append(['', 'Adjusted R Square', format_number(model.rsquared_adj)])
                    summary_data.append([f"S{idx}SE", 'Standard Error of the Regression', format_number(model.scale ** 0.5)])
                    summary_data.append(['', 'Observations', format_number(int(model.nobs))])

                    # Add ANOVA table
                    summary_data.append(['', 'ANOVA', ''])
                    summary_data.append(['', '', 'df', 'SS', 'MS', 'F', 'Significance F'])
                    for index, row in anova_table.iterrows():
                        formatted_row = [format_number(item) if isinstance(item, (int, float)) else item for item in row.tolist()]
                        summary_data.append(['', str(index)] + formatted_row)
